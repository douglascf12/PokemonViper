...
class PokemonAPI {
    ...
    func getPokemons(completion: @escaping ([PokemonAPIModel]) -> ()) {
        ...
        request(url: pokemonURL) { (json) in
            guard let _json = json,
                  let pokemons = Mapper<PokemonAPIModel>().mapArray(JSONObject: _json["results"]) else {
                completion([])
                return
            }
            completion(pokemons)
    ...
    func getPokemon(id: String, completion: @escaping (PokemonDetailsAPIModel) -> ()) {
        ...
        request(url: pokemonURL) { (json) in
            guard let _json = json,
                  let pokemon = Mapper<PokemonDetailsAPIModel>().map(JSON: _json ) else {
                return
            }
            completion(pokemon)
        }
